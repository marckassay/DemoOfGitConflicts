...
function Switch-ProjectLocation
{
    ...
    {
        'HostDirectory' {
            $LocationPath = $Env:LOCALAPPDATA
        }
        'ProjectDirectory' {
            $LocationPath = $Env:LOCALAPPDATA+"\"+$Settings.ProjectName
        }
        'SubRepoA' {
            $LocationPath = $Env:LOCALAPPDATA+"\"+$Settings.ProjectName+"\"+$User.Alice
        }
        'SubRepoB' {
            $LocationPath = $Env:LOCALAPPDATA+"\"+$Settings.ProjectName+"\"+$User.Bob
        }
    }

    if ($Clean) {
        Remove-Item $LocationPath -Force -Recurse -ErrorAction 'SilentlyContinue'
    }

    $PathExists = Test-Path -Path $LocationPath
    if($PathExists -eq $False)
    {
        New-Item -Path $LocationPath -ItemType 'Directory'
    }
